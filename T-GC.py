...
@app.cell
def _(F_R_gamma, Q_Y, c, f_R_gamma, f_T_weibull, uQ_Y):
    def w_gc(x, alpha, theta, mu, sigma, lam): # Normal - Gamma {Cauchy}
        return f_T_weibull(Q_Y(F_R_gamma(x, alpha, theta), lam), c) * uQ_Y(F_R_gamma(x, alpha, theta), lam) * f_R_gamma(x, alpha, theta)
    return
...
def _(calc_params_NKC, norm, np, skewnorm):
    ...
    return bimod_params, lskew_params, norm1_params, rskew_params, x


@app.cell
def _(mo):
    mo_a = mo.ui.slider(start=0.001, stop=3, step=0.001, value=0.001, label=fr'$a$')
    mo_b = mo.ui.slider(start=0.001, stop=5, step=0.001, value=0.001, label=fr'$b$')
    ...
    mo_lam = mo.ui.slider(start=0.1, stop=100, step=0.1, value=0.1, label=fr'$\lambda$')
    ...
    #mo_c = mo.ui.slider(start=0, stop=100, step=0.001, value=0.001, label=fr'$c$')


    mo.hstack([mo_a, mo_b, mo_lam, mo_mu, mo_sigma], justify='center', align='center', gap=5)
    return mo_a, mo_b, mo_lam, mo_mu, mo_sigma
...
def _(
    ...
    ax.plot(x, n_kc(x, mo_a.value, mo_b.value, mo_lam.value, mo_mu.value, mo_sigma.value), lw=3)
